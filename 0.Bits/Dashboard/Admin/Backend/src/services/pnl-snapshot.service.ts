...
export async function computeDailyPnlSnapshot(currency: string, date: Date) {
  ...
  const result = await fifoV2.computeForCurrency(currency, dayStart, dayEnd);

  return prisma.pnlSnapshot.upsert({
    where: {
      date_currency: { date: dayStart, currency },
    },
    create: {
      date: dayStart,
      currency,
...
export async function backfillPnlSnapshots(days: number = 180, currency: string = "EUR") {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    try {
      await computeDailyPnlSnapshot(currency, date);
      count++;
    } catch (err) {
 ...
 */
export async function getPnlTimeSeries(currency: string = "EUR", days: number = 180) {
